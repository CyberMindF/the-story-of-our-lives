  ...
  for (const page of pages) {
    ...
    const suggestLink = page.hideSuggestLink
      ...
      : `          <a href="${base}suggerimenti/" class="suggest-link" aria-label="Manda un suggerimento" title="Suggerisci">\n`
        ...
        + `          </a>`;
    const shellIdAttr = page.shellId ? ` id="${page.shellId}"` : "";
    // La maggior parte delle pagine usa la logica comune di saluto/tema/logout in
    // assets/js/world/main.js; una pagina che reimplementa già quella stessa logica per conto
    // suo (es. il cruciverba, con la sua gestione dedicata) imposta skipWorldScript per non
    // caricare lo script due volte e duplicare listener sugli stessi elementi condivisi.
    const worldScript = page.skipWorldScript
      ? ""
      : `    <script type="module" src="${base}assets/js/world/main.js?v=2"></script>`;
    ...
    const html = template
      ...
      .replaceAll("{{SHELL_ID_ATTR}}", shellIdAttr)
      ...
      .replaceAll("{{WORLD_SCRIPT}}", worldScript)
