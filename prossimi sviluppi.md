...
- [x] #e17 — Modalità di lettura pulita: query param `?clean=1`, letto da un nuovo `CleanModeService` condiviso (`web/src/app/core/clean-mode.service.ts`) invece che duplicato pagina per pagina. `AppShell` lo consulta da sé e nasconde header e back-link quando attivo (nuovi getter `resolvedShowHeader`/`resolvedShowBackLink` che combinano gli `@Input` esistenti con lo stato del servizio), senza bisogno di passare un `@Input` in più da ogni pagina. Le Lettere, primo caso d'uso, iniettano lo stesso servizio per nascondere anche il form di scrittura in clean mode; il resto della pagina (archivio, apertura/lettura della lettera nel dialog) resta invariato. **Aggiunto in corsa** (Rory ha chiesto un punto d'attivazione, non solo l'URL a mano): interruttore persistito (`localStorage`, chiave `noi-clean-mode-v1`) nella Stanza dei Bottoni, che forza la modalità su tutte le pagine senza bisogno del query param — per ora visibile solo agli admin (`authService.isAdmin()`), il toggle resta comunque leggibile/spegnibile anche a chrome nascosto perché vive nel contenuto della pagina, non nell'header che sparisce. `CleanModeService.active` è diventato un `computed` (query param OR interruttore forzato) invece di un booleano semplice. Verificato con Playwright (account admin di prova poi ripulito da D1 locale): `/lettere` normale invariata, `/lettere?clean=1` senza header/back-link/form, apertura di una lettera nel dialog ancora funzionante in clean mode, interruttore in Stanza dei Bottoni che forza clean mode anche su `/lettere` senza query param e lo toglie di nuovo.
