...
- [ ] **Introdurre i temi selezionabili su tutto il Mondo Bianco.** Rory non è ancora sicuro se/quando farlo: estendere il sistema di variabili condiviso (già usato dal cruciverba) a tutte le pagine del mondo, e mostrare lo stesso selettore tema ovunque, non solo lì. Il Mappamondo racconta che "il Mondo Bianco può cambiare come vogliamo", quindi avrebbe senso narrativo oltre che tecnico — ma è un refactoring che tocca ogni `assets/css/pages/*.css` del mondo, da valutare con calma.

- [ ] **Valutare un sistema di template/include per l'HTML.** Segnalato il 09/08/2026: il sito è statico puro, ogni pagina contiene l'header (e altre parti condivise) copiato per intero — nessun sistema di componenti/partial. Qualsiasi modifica a un pezzo condiviso richiede di toccare ogni pagina singolarmente (è già successo con la lampadina dei Suggerimenti, 15 file). Da valutare dopo aver "portato a casa" il sito: un piccolo passo di build che assembla header/footer condivisi + contenuto specifico di pagina, senza cambiare l'hosting statico.
