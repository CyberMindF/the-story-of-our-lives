...
- [X] **Risolvere la frattura stilistica tra Mondo Bianco e cruciverba.** Fatto il 09/08/2026, in due parti:
  ...
  2. **Colori**: nuovo tema fisso `the-white-world` in `themes.css`, con le stesse variabili di colore già usate (hardcoded) dalle altre pagine. Il selettore dei 4 temi (Ocean/Velvet/Red of You/Green of Me) era stato **tolto dal cruciverba** in questo primo passaggio, per non avere un'unica pagina con un selettore che il resto del sito non aveva ancora — poi reintrodotto quando il selettore è arrivato su tutta la piattaforma (vedi item sotto). CSS morto rimosso (`.hero`, `.theme-bar`, `.theme-switcher`, `.theme-chip*`, `.theme-toast`, `.crossword-back-link`).
...
- [X] **Introdurre i temi selezionabili su tutto il Mondo Bianco.** Fatto il 09/08/2026: tutte le 17 pagine del Mondo Bianco, cruciverba compreso, mostrano lo stesso selettore tema — 5 pallini colorati (Notte/Ocean/Velvet/Red of You/Green of Me) — nella userbar, con `the-white-world` come tema di default. Stessa `localStorage` key (`noi-crossword-theme-v15`) già condivisa da tutte le pagine, quindi la scelta resta coerente ovunque. Il cruciverba inizialmente era stato lasciato escluso (vedi item sopra e sotto), poi corretto lo stesso giorno perché "tutta la piattaforma" includeva anche lui. Nota: per ora cambia solo `--focus-color`/il tint del cielo stellato, non ancora i colori hardcoded di ogni pagina (quello resta un refactoring più profondo, non ancora fatto).

- [X] **Valutare un sistema di template/include per l'HTML.** Fatto il 09/08/2026: aggiunto un generatore statico (`scripts/build-world-pages.mjs` + `templates/world-page.html` + `templates/pages/*.content.html` + `scripts/world-pages.manifest.mjs`) che assembla header/userbar/footer condivisi con il contenuto specifico di ogni pagina e scrive gli `index.html` finali nel repo — nessun cambiamento all'hosting statico di Cloudflare Pages, i file generati sono commit normali. Applicato a tutte le 16 pagine del Mondo Bianco (comprese le 4 del GDR). Il cruciverba resta volutamente fuori da **questo** sistema di generazione: ha una shell diversa (`.app-shell`) ed è l'unica pagina con quella struttura, quindi non c'è duplicazione di markup da eliminare lì — ma condivide comunque `.place-header`/`.place-userbar`/il selettore tema a mano (vedi item sopra). D'ora in poi, per modificare l'header/footer condiviso delle 16 pagine templated si tocca `templates/world-page.html` e si rilancia `node scripts/build-world-pages.mjs`, non più ogni pagina a mano. Rimosse anche le ~9 regole CSS `.XXX-home-link` duplicate (una identica per pagina) in favore di un'unica `.place-bottom-link` in `world-shell.css`. Verificato con Playwright su tutte le 17 pagine: 0 errori console, switcher funzionante e persistente, invio moduli (maga, appunti, turno, suggerimenti) e gioco del cruciverba tutti confermati funzionanti.
